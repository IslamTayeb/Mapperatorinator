...
Use `python utils/summarize_active_prefix_diagnostics.py PROFILE.json --label main_generation --json-output active_diagnostics.json` to aggregate per-record active-prefix counters, CUDA graph capture/replay counts, bucket usage, and per-logits-processor wall time without writing ad hoc parsers for every DCC run.
