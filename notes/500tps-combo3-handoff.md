...
| **§38** | Tier-2 relaxed fused decoder step (7-kernel; fp32 acc) | **PARKED** behind §46–§50 — quality PASS / perf MISS sealed |
| **§44** | TIER1 evidence pack harness (30×3 + KS + canary) | **HARNESS READY** — fire only on ≥**384** scout; no 500 claim; §39 not duplicated |
...
| **§45** | Combined turbo integrator (44.52 scout) | **EXPLAINED / REOPENED** — deep-research package; not dead-end |
| **§46** | W-BASE baseline glue v2 | **STOP_NO_PROMOTE** — `50150069` exact; −0.003≪0.15 ms/tok |
| **§47** | W-KV keep-accepted-KV + O(1) rollback | **GATES PASS** — canary `50150072` / scout `50150073`; ready for §48/§49 merge |
| **§48** | W-VG graph-native K=γ verify | **STOP_KILL** — `50150290` c_verify **3.075 ms** / **1.669×** |
| **§49** | W-DG graphed draft chain | **MISS_UNDER_KILL** — `50150142` γ=3 **1.257 ms** (≤1.2 miss; &gt;2 kill no) |
| **§50** | W-ACC config/margin | **QUEUED** after cycle &lt;10 ms |
| **Post-rung** | Ceiling decision | **(A)** — keep-KV+DG ceilings **461.68 / 554.02** (≥420); merge §47+§49 → one path-hit scout; §51 EAGLE parked |
| **§51** | EAGLE-style draft head | **OPEN/SCAFFOLD** `codex/turbo-eagle-draft-head` (after §52) |
...
22. **§45 combined turbo EXPLAINED / REOPENED** — scout **`50148770`** main_tps **44.52** was unoptimized sampled path (verify graphs hard-off K>1; TEACHER_ALIGNED inert under do_sample; crop-to-L rebuild; eager draft×4; host syncs/sorts). Ceilings: **~570** if W-KV+W-VG+W-DG land; **~310** if any miss; baseline-glue **~540** no-speculation. Handoff: `notes/500tps-section45-handoff.md`. Tip still `55949274` / **366.11**.
23. **TURBO DEEP-RESEARCH PACKAGE** — **RULING:** keep-accepted-KV fp16 ULP = DOCUMENTED DRIFT under §34 for *sampled* turbo (TIER1 KS); greedy TIER1a keeps crop-rebuild/aligned-Q1. Open stubs **§46–§50** (W-BASE/W-KV/W-VG/W-DG/W-ACC). **§38 PARKED**. Process: path-hit counters; recompute ceiling; kill criteria; no §44 until ≥384. First ≥450 scout carries. No merge.
24. **§46 W-BASE STOP_NO_PROMOTE** — `codex/exact-baseline-glue-v2` @ **`3987bd5c`**; smoke **`50150069`** exact PASS (1322 tok/RNG/hits) but **−0.003 ≪ 0.15** ms/tok; no scout. Handoff: `notes/500tps-section46-handoff.md`. Tip still `55949274` / **366.11**.
25. **§47 W-KV GATES PASS** — tip `d3cd6939`; canary **`50150072` PASS**; scout **`50150073`** teacher_forwards/cycle **1.0**, accepted_reforwards **0**, median cycle **32.61 ms** (−12.39 vs §45 ~45). Summary Slurm exit false-negative (counters on `generation[]`). Ready for integrator merge with §48/§49 — **no merge**. Handoff: `notes/500tps-section47-handoff.md`. Tip still `55949274` / **366.11**.
26. **§48 W-VG STOP_KILL** — `50150290` graph_native c_verify **3.075 ms** / **1.669×**. Handoff: `notes/500tps-section48-handoff.md`.
