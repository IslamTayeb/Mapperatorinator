...
## 45. Combined turbo perf integrator (§41 verify + §43 1-layer γ=3) — **STOP_DEAD_END**
...
| Status | **STOP_DEAD_END** — Track C speculative closed for 500 path |
...
| Scout | **`50148770`** main_tps **44.52** / main_model **155.36 s** / **6917** tok; sustained median ~**23 ms/tok** |
| Root cause | (1) crop-to-L + full teacher+draft rebuild every step (keep-prefix broke TIER1a @110); (2) §41 c_verify **1.686×** miss; (3) §42 c_draft **0.36×** miss / not in tip; (4) profiler dropped turbo_* stats |
| Structural bound | K=1 γ=3 E≈1.97: no-rebuild ceiling **~310 TPS**; with rebuild **~208 TPS** — both ≪ tip **366** / ≥384 (`utils/s45_turbo_structural_bound.py`) |
| §46 | **not submitted** — no path to ≥384 without meeting both cost gates |
| Handoff | `notes/500tps-section45-handoff.md` |
| Campaign tip | still `55949274` / FP16 **366.11** — **no 500 claim**; **no tip graduate** |
| Decision | **STOP** Track C speculative; next **§38 TIER2 fused step** (reopen spec only if c_verify≤1.2× and c_draft≤0.15× + TIER1a-safe keep-prefix) |
| Not this lever | §39 / turbo_mixed; tip graduation; merge to main |
| Ledger rule | Own section only |

## 46. Turbo speculative re-scout — **NOT STARTED / SUPERSEDED by §45 dead-end**

| Field | Value |
| --- | --- |
| What | Planned FP16 SALVALAI re-scout after §45 diagnose fix |
| Status | **SKIPPED** — structural dead-end; no ≥384 path at measured gates |
| Decision | Do not queue; prefer §38 |
| Ledger rule | Own section only |
