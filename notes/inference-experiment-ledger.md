...
This is a family-level decision index, not a chronological lab notebook. Exact
job transcripts remain in [`archive/`](archive/) and Git history.

| Family | Decision | Durable lesson | Revisit only when |
| --- | --- | --- | --- |
| Optimized single | Accepted, opt-in. SALVALAI FP32/SDPA reached `270.475 tok/s` with exact tokens and output (`49230082`). | Persistent session state, active-prefix graphs, stateful monotonic processing, and q1 kernels form the useful production stack. | A current exact full-song comparison demonstrates a further end-to-end gain. |
...
| Measurement/verifiers | Retained. Exact logits/cache/RNG, persistent-session, weighted-prefix, output-hash, and queue-state tools remain useful. | Infrastructure may graduate even when its candidate fails. | Remove only when an equivalent stronger gate replaces it. |
| Configuration and Python-loop tweaks | Cut. Compile modes, cache variants, preparation shortcuts, stopping specializations, and similar local changes were flat, unstable, or below `5%`. | Small local timings rarely survive full-song wall. | A current profile shows the same region is again target-sized. |
| Narrow kernels and fusions | Mostly cut. Isolated linear, MLP, attention, replay-shell, and cache-copy wins lacked enough weighted end-to-end headroom. | Weight traffic, launch overhead, and prefix distribution matter more than a best-case microbenchmark. | A broader real-prefix ceiling clears the end-to-end bar before runtime work. |
...
| Practical two-song queue | Rejected on `codex/phase-pipeline-scout` (`49627068`). Outputs were exact, but only `91` steps paired versus `2,625` private steps; candidate was about `75 tok/s` and took `2.6-3.8x` optimized serial wall. | Sparse compatibility plus an expensive fallback makes this queue shape fundamentally negative. | A materially different shape first wins direct reciprocal N=2 wall above the optimized-single frontier. |
