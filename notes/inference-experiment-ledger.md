...
| --- | --- | --- | --- |
| Optimized single | Accepted, opt-in. SALVALAI FP32/SDPA reached `270.475 tok/s` with exact tokens and output (`49230082`). Minimal-runtime smoke `49708641` / `c3fe6bc` preserved exact output with reciprocal main TPS at `+1.31%` / `-0.68%`. | Persistent session state, active-prefix graphs, stateful monotonic processing, and q1 kernels form the useful production stack. | A current exact full-song comparison demonstrates a further end-to-end gain. |
