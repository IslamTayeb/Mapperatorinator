...
| B8 16-step changing-prefix verifier | setup-only `49547750` / `aec60c6` | Added independent-B1-vs-merged-B8 per-step logits/top-k, processed-score, token, private-RNG, and active self/cross-cache gates plus model-free forced-EOS/inactive/dummy tests. The Slurm attempt reached a 2080 Ti and clean exact commit but failed before model load because the DCC environment lacks `pytest`; no report or physics measurement exists. | Accept verifier infrastructure only. The job is neither a correctness nor throughput result. Remove the redundant in-job pytest dependency, then rerun only with explicit approval; still do not wire runtime/scheduler code. |
