...
| Relaxed FP16/native-prefix single | Rejected at the current-main component gate (`49715303`). All cache, finiteness, ownership, RNG, and self-repeat gates passed with documented baseline drift, but FP16 framework projected `31.699s` and FP16 native self+cross projected `32.213s` versus the `25.419s` target. Native was `6.40%` slower than framework in FP16 and `20.15%` slower in FP32. | Turing FP16 storage plus shared framework linears does not make this q_len=1 decoder faster; the custom native q1 prefix worsens the dominant long-prefix buckets. Charge graph setup separately and weight the current 12-bucket distribution. | A materially different half-precision implementation first shows at least `5%` weighted full-model component headroom, or the GPU/framework changes. |
