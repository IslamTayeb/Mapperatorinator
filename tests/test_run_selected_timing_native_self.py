...
from utils import run_selected_timing_native_self as runner
...
def _selected_evidence(mode: str) -> dict:
    return {
...
def test_runner_scopes_selected_main_and_exact_timing_owners(monkeypatch, tmp_path) -> None:
    ...
    def fake_selected(config_name, overrides, output_init_json, *, mode):
        ...
        output_init_json.write_text(
            json.dumps(_selected_evidence(mode)),
            encoding="utf-8",
        )
    ...
    timing = payload["timing_native_self"]
    assert timing["version"] == engine.TIMING_NATIVE_SELF_VERSION
    assert timing["result_class"] == "exact"
    assert timing["exactness_claim"] is True
    assert timing["incremental_control"] == runner.SELECTED_MAIN_COMPOSITION
    assert payload["combined_runtime"] == runner.SELECTED_MAIN_COMPOSITION


def test_runner_rejects_topology_drift_and_restores_loader(monkeypatch, tmp_path) -> None:
    bindings = [_binding(), _binding()]
    load_index = 0
    ...
    def fake_selected(config_name, overrides, output_init_json, *, mode):
        inference.load_model_with_engine(auto_select_gamemode_model=True)
        inference.load_model_with_engine(auto_select_gamemode_model=False)
        evidence = _selected_evidence(mode)
        evidence["combined_runtime"] = "wrong-topology"
        output_init_json.write_text(json.dumps(evidence), encoding="utf-8")

    monkeypatch.setattr(runner, "run_selected", fake_selected)

    with pytest.raises(RuntimeError, match="composition is missing"):
...
def test_runner_restores_loader_when_nested_composition_fails(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(runner, "run_selected", fail)
