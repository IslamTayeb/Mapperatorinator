...
K1_INT8_COMPOSED_WRAPPER = (
    ROOT
    / "scripts/dcc/verify_k4_shared_rope_k1_int8_stage_precision_reciprocal.sbatch"
)
...
def test_stage_precision_wrapper_has_valid_bash_syntax():
    ...
    subprocess.run(["bash", "-n", str(K1_INT8_COMPOSED_WRAPPER)], check=True)
...
def test_k1_int8_wrapper_changes_only_timing_precision() -> None:
    source = K1_INT8_COMPOSED_WRAPPER.read_text(encoding="utf-8")
    general = WRAPPER.read_text(encoding="utf-8")

    assert "run_k4_shared_rope_k1_int8_stage_aware_control.py" in source
    assert "run_k4_shared_rope_k1_int8_stage_precision_hybrid.py" in source
    assert "REQUIRE_K1_INT8_STAGE_COMPOSITION=true" in source
    assert "FIXED_TIMING_TOKENS=821" in source
    assert "FIXED_MAIN_TOKENS=8294" in source
    assert "validate_k1_int8_stage_precision_profile.py" in general
    assert "--require-int8-mlp --require-graph-remainders" in general
    assert "preload_int8_mlp_extension" in general
