...
@pytest.mark.parametrize("role", ["control", "candidate"])
def test_validator_requires_k1_int8_composition_evidence(role: str) -> None:
    payload = _payload(role)
    payload["combined_runtime"] = "k1-int8-stage-test-v1"
    payload["graph_remainders"] = True
    payload["int8_mlp_overlay"] = {
    ...
    report = validate(
        payload,
        role=role,
        combined_runtime="k1-int8-stage-test-v1",
        require_int8_mlp=True,
        require_graph_remainders=True,
    )

    assert report["graph_remainders"] is True
    assert report["int8_mlp_overlay"] is True
