#!/usr/bin/env python3
"""Reciprocal exact decode cast/copy elimination on shared-RoPE + device-state tip."""
...
def _activation_payload(
    *,
    ...
    if mode not in {"baseline", "candidate"}:
        raise ValueError("decode-cast-copy mode must be baseline or candidate")
    if isinstance(calls, bool) or not isinstance(calls, int) or calls < 0:
        raise RuntimeError("decode-cast-copy decode-loop count is invalid")
    if calls <= 0:
        raise RuntimeError("decode-cast-copy requires device-sequence-state decode calls")
    return {
        "schema_version": 1,
        "role": mode,
        "candidate": "exact_device_state_plus_decode_cast_copy",
        ...
        "decode_cast_copy_enabled": mode == "candidate",
    ...
    *,
    ...
    try:
        with ExitStack() as stack:
            ...
            if mode == "candidate":
                stack.enter_context(decode_cast_copy_candidate_context())
    ...
    if rng_after_seed is None:
        raise RuntimeError("decode-cast-copy inference never initialized the RNG environment")
