#!/usr/bin/env python3
"""Reciprocal exact whole-token-step CUDA graph candidate on tip."""
...
def _activation_payload(
    *,
    ...
    whole_token_hits: int,
) -> dict[str, object]:
    if mode not in {"baseline", "candidate"}:
        raise ValueError("whole-token-step mode must be baseline or candidate")
    if isinstance(calls, bool) or not isinstance(calls, int) or calls < 0:
        raise RuntimeError("whole-token-step decode-loop count is invalid")
    if calls <= 0:
        raise RuntimeError(
            "whole-token-step requires device-sequence-state decode calls"
        )
    if mode == "candidate" and whole_token_hits <= 0:
        raise RuntimeError(
            "whole-token-step candidate requires capture/replay hits > 0"
        )
    return {
        "schema_version": 1,
        "role": mode,
        "candidate": "exact_device_state_plus_whole_token_step_cuda_graph",
        ...
        "whole_token_step_cuda_graph_enabled": mode == "candidate",
        "whole_token_step_cuda_graph_hits": (
            whole_token_hits if mode == "candidate" else 0
        ),
    ...
    *,
    ...
    try:
        with ExitStack() as stack:
            ...
            if mode == "candidate":
                stack.enter_context(whole_token_step_cuda_graph_candidate_context())
            run_shared_rope(config_name, overrides, rope_evidence_path)
            hits = int(whole_token_step_cuda_graph_hits())
    ...
    if rng_after_seed is None:
        raise RuntimeError(
            "whole-token-step inference never initialized the RNG environment"
        )
    payload = _activation_payload(
        ...
        rng_after_seed=rng_after_seed,
        rng_after_inference=rng_after_inference,
        whole_token_hits=hits if mode == "candidate" else 0,
    )
