#!/usr/bin/env python3
"""Reciprocal runner: tip (shared-RoPE + device state) ± self RMSNorm+Wqkv fuse."""
...
def run(
    *,
    ...
    from osuT5.osuT5.inference.optimized.scout.self_norm_wqkv import (
    ...
    fuse_context = (
        self_norm_wqkv_fusion_candidate_context()
        if mode == "candidate"
        else nullcontext(SelfNormWqkvFusionActivation())
    )
    ...
    if rng_after_seed is None:
        raise RuntimeError("self-norm Wqkv inference never initialized RNG")
