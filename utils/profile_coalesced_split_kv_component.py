"""Size the opt-in coalesced split-KV partial kernel on live SALVALAI tensors."""
...
def profile_component(
    ...
    *,
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("coalesced split-KV profiler requires CUDA")
    if torch.cuda.get_device_capability() != (7, 5):
        raise RuntimeError("coalesced split-KV profiler requires SM75")
    ...
    from osuT5.osuT5.inference.optimized.kernels.q1_attention import (
        ...
        native_q1_rope_cache_attention_coalesced,
    ...
    for prefix, accepted in entries.items():
        ...
        def candidate_call() -> torch.Tensor:
            return native_q1_rope_cache_attention_coalesced(
                qkv, keys, values, cos, sin, capture.cache_position, mask, prefix
            )

        calls = {"accepted": accepted_call, "coalesced": candidate_call}
        verifiers = {
    ...
    return {
        "schema_version": 1,
        "metadata": {
            "candidate": "sm75_split_kv_8_coalesced_partial",
            "commit": _git_head(),
            "precision": "fp32",
            "bucket_mode": "all",
...
def main() -> None:
    ...
    if not report["summary"]["promotion_pass"]:
        raise SystemExit("STOP_COALESCED_SPLIT_KV_COMPONENT")
