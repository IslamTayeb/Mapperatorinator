#!/usr/bin/env python3
"""Reciprocal exact compiled self Wqkv/Wo on shared-RoPE + device-state tip."""
...
def _activation_payload(
    *,
    ...
    if mode not in {"baseline", "candidate"}:
        raise ValueError("compiled-self-proj mode must be baseline or candidate")
    if isinstance(calls, bool) or not isinstance(calls, int) or calls < 0:
        raise RuntimeError("compiled-self-proj decode-loop count is invalid")
    if calls <= 0:
        raise RuntimeError("compiled-self-proj requires device-sequence-state decode calls")
    return {
        "schema_version": 1,
        "role": mode,
...
def run(
    *,
    ...
    if rng_after_seed is None:
        raise RuntimeError("compiled-self-proj inference never initialized the RNG environment")
