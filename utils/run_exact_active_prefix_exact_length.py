#!/usr/bin/env python3
"""Reciprocal exact active-prefix exact-length (bucket=1) override on tip."""
...
def _activation_payload(
    *,
    ...
    if mode not in {"baseline", "candidate"}:
        raise ValueError("active-prefix-exact-length mode must be baseline or candidate")
    if isinstance(calls, bool) or not isinstance(calls, int) or calls < 0:
        raise RuntimeError("active-prefix-exact-length decode-loop count is invalid")
    if calls <= 0:
        raise RuntimeError(
            "active-prefix-exact-length requires device-sequence-state decode calls"
        )
    return {
        "schema_version": 1,
        "role": mode,
        "candidate": "exact_device_state_plus_active_prefix_exact_length",
        ...
        "active_prefix_exact_length_enabled": mode == "candidate",
        "active_prefix_exact_length_size": bucket_size if mode == "candidate" else None,
...
def run(
    *,
    ...
    from osuT5.osuT5.inference.optimized.kernels.active_prefix_exact_length import (
    ...
    try:
        with ExitStack() as stack:
            ...
            if mode == "candidate":
                stack.enter_context(active_prefix_exact_length_candidate_context())
    ...
    if rng_after_seed is None:
        raise RuntimeError(
            "active-prefix-exact-length inference never initialized the RNG environment"
        )
    payload = _activation_payload(
        ...
        rng_after_seed=rng_after_seed,
        rng_after_inference=rng_after_inference,
        bucket_size=CANDIDATE_ACTIVE_PREFIX_BUCKET_SIZE,
    )
