#!/usr/bin/env python3
"""Reciprocal runner: tip (shared-RoPE + device state) ± self-attn Wo one-token linear (no residual)."""
...
def run(
    *,
    ...
    from osuT5.osuT5.inference.optimized.scout.self_wo_linear import (
    ...
    wo_context = (
        self_wo_linear_candidate_context()
        if mode == "candidate"
        else nullcontext(SelfWoLinearActivation())
    )
    ...
    try:
        with _stacked(
            device_sequence_state_candidate_context(),
            wo_context,
        ) as (device_activation, wo_activation):
    ...
    if rng_after_seed is None:
        raise RuntimeError("self-wo linear inference never initialized RNG")
    payload = _activation_payload(
        mode=mode,
        decode_loop_calls=int(device_activation.calls),
        wo_calls=int(wo_activation.calls),
        rng_after_seed=rng_after_seed,
        rng_after_inference=rng_after_inference,
    )
