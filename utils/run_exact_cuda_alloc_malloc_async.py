#!/usr/bin/env python3
"""Reciprocal tip + PYTORCH_CUDA_ALLOC_CONF=backend:cudaMallocAsync candidate."""
...
CANDIDATE_ALLOC_CONF = "backend:cudaMallocAsync"
...
def _require_mode_alloc_conf(mode: str) -> str | None:
    ...
    if mode == "candidate":
        if conf != CANDIDATE_ALLOC_CONF:
            raise RuntimeError(
                "cuda-alloc-malloc-async candidate requires "
                f"PYTORCH_CUDA_ALLOC_CONF={CANDIDATE_ALLOC_CONF!r}, got {conf!r}"
            )
        return conf
    if conf is not None:
        raise RuntimeError(
            "cuda-alloc-malloc-async baseline requires unset "
...
def _activation_payload(
    *,
    ...
    if mode not in {"baseline", "candidate"}:
        raise ValueError("cuda-alloc-malloc-async mode must be baseline or candidate")
    if isinstance(calls, bool) or not isinstance(calls, int) or calls < 0:
        raise RuntimeError("cuda-alloc-malloc-async decode-loop count is invalid")
    if calls <= 0:
        raise RuntimeError(
            "cuda-alloc-malloc-async requires device-sequence-state decode calls"
        )
    ...
    return {
        "schema_version": 1,
        "role": mode,
        "candidate": "exact_device_state_plus_cuda_alloc_malloc_async",
        "enabled": enabled,
        ...
        "cuda_alloc_malloc_async_enabled": enabled,
...
def run(
    *,
    ...
    if rng_after_seed is None:
        raise RuntimeError(
            "cuda-alloc-malloc-async inference never initialized the RNG environment"
        )
