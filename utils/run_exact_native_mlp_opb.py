#!/usr/bin/env python3
"""Reciprocal exact native cross+MLP outputs_per_block occupancy on tip."""
...
def _activation_payload(
    *,
    ...
    outputs_per_block: int | None,
) -> dict[str, object]:
    if mode not in {"baseline", "candidate"}:
        raise ValueError("native-mlp-opb mode must be baseline or candidate")
    if isinstance(calls, bool) or not isinstance(calls, int) or calls < 0:
        raise RuntimeError("native-mlp-opb decode-loop count is invalid")
    if calls <= 0:
        raise RuntimeError(
            "native-mlp-opb requires device-sequence-state decode calls"
        )
    return {
        "schema_version": 1,
        "role": mode,
        "candidate": "exact_device_state_plus_native_mlp_outputs_per_block",
        ...
        "native_mlp_outputs_per_block_enabled": mode == "candidate",
        "native_mlp_outputs_per_block_size": (
            outputs_per_block if mode == "candidate" else None
        ),
...
def run(
    *,
    ...
    from osuT5.osuT5.inference.optimized.kernels.native_mlp_outputs_per_block import (
    ...
    try:
        with ExitStack() as stack:
            ...
            if mode == "candidate":
                stack.enter_context(native_mlp_outputs_per_block_candidate_context())
    ...
    if rng_after_seed is None:
        raise RuntimeError(
            "native-mlp-opb inference never initialized the RNG environment"
        )
    payload = _activation_payload(
        ...
        rng_after_seed=rng_after_seed,
        rng_after_inference=rng_after_inference,
        outputs_per_block=CANDIDATE_OUTPUTS_PER_BLOCK,
    )
