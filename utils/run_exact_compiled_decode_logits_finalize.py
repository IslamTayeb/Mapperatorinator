#!/usr/bin/env python3
"""Reciprocal exact compiled decode logits finalize on shared-RoPE + device-state tip."""
...
def _activation_payload(
    *,
    ...
    if mode not in {"baseline", "candidate"}:
        raise ValueError("compiled-decode-logits-finalize mode must be baseline or candidate")
    if isinstance(calls, bool) or not isinstance(calls, int) or calls < 0:
        raise RuntimeError("compiled-decode-logits-finalize decode-loop count is invalid")
    if calls <= 0:
        raise RuntimeError("compiled-decode-logits-finalize requires device-sequence-state decode calls")
    return {
        "schema_version": 1,
        "role": mode,
        "candidate": "exact_device_state_plus_compiled_decode_logits_finalize",
        ...
        "compiled_decode_logits_finalize_enabled": mode == "candidate",
...
def run(
    *,
    ...
    from osuT5.osuT5.inference.optimized.kernels.compiled_decode_logits_finalize_activation import (
    ...
    try:
        with ExitStack() as stack:
            ...
            if mode == "candidate":
                stack.enter_context(compiled_decode_logits_finalize_candidate_context())
    ...
    if rng_after_seed is None:
        raise RuntimeError("compiled-decode-logits-finalize inference never initialized the RNG environment")
