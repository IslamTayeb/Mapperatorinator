...
def run(
    ...
    *,
    ...
    if not isinstance(initializer_name, str) or not initializer_name:
        raise ValueError("stage-precision initializer name must be non-empty")
    ...
    def hybrid_loader(*loader_args, **loader_kwargs):
        ...
        if load_count == 0:
            ...
            if not callable(initializer):
                raise RuntimeError(
                    "main runtime lacks requested mixed-weight initialization "
    ...
    try:
        ...
        initialization["graph_remainders"] = graph_remainders
