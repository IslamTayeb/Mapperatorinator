#!/usr/bin/env python3
"""Reciprocal exact self out_proj+residual fuse on shared-RoPE + device-state tip."""
...
def _activation_payload(
    *,
    ...
    if mode not in {"baseline", "candidate"}:
        raise ValueError("self-out-residual mode must be baseline or candidate")
    if isinstance(calls, bool) or not isinstance(calls, int) or calls < 0:
        raise RuntimeError("self-out-residual decode-loop count is invalid")
    if calls <= 0:
        raise RuntimeError("self-out-residual requires device-sequence-state decode calls")
    return {
        "schema_version": 1,
        "role": mode,
        "candidate": "exact_device_state_plus_self_out_residual",
        ...
        "self_out_residual_enabled": mode == "candidate",
    ...
    *,
    ...
    try:
        with ExitStack() as stack:
            ...
            if mode == "candidate":
                stack.enter_context(self_out_residual_candidate_context())
    ...
    if rng_after_seed is None:
        raise RuntimeError("self-out-residual inference never initialized the RNG environment")
