#!/usr/bin/env python3
"""Reciprocal exact q1 mask workspace on shared-RoPE + device-state tip."""
...
def _activation_payload(
    *,
    ...
    if mode not in {"baseline", "candidate"}:
        raise ValueError("q1-mask-workspace mode must be baseline or candidate")
    if isinstance(calls, bool) or not isinstance(calls, int) or calls < 0:
        raise RuntimeError("q1-mask-workspace decode-loop count is invalid")
    if calls <= 0:
        raise RuntimeError("q1-mask-workspace requires device-sequence-state decode calls")
    return {
        "schema_version": 1,
        "role": mode,
        "candidate": "exact_device_state_plus_q1_mask_workspace",
        ...
        "q1_mask_workspace_enabled": mode == "candidate",
...
def run(
    *,
    ...
    from osuT5.osuT5.inference.optimized.kernels.q1_mask_workspace import (
    ...
    try:
        with ExitStack() as stack:
            ...
            if mode == "candidate":
                stack.enter_context(q1_mask_workspace_candidate_context())
    ...
    if rng_after_seed is None:
        raise RuntimeError("q1-mask-workspace inference never initialized the RNG environment")
