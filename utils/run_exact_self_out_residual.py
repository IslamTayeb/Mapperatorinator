#!/usr/bin/env python3
"""Reciprocal runner: tip (shared-RoPE + device state) ± self-out residual fusion."""
...
def run(
    *,
    ...
    fusion_context = (
        self_out_residual_fusion_candidate_context()
        if mode == "candidate"
        else nullcontext(SelfOutResidualFusionActivation())
    )
    ...
    try:
        with _stacked(
            device_sequence_state_candidate_context(),
            fusion_context,
        ) as (device_activation, fusion_activation):
    ...
    if rng_after_seed is None:
        raise RuntimeError("self-out fusion inference never initialized RNG")
    payload = _activation_payload(
        mode=mode,
        decode_loop_calls=int(device_activation.calls),
        fusion_calls=int(fusion_activation.calls),
        rng_after_seed=rng_after_seed,
        rng_after_inference=rng_after_inference,
    )
