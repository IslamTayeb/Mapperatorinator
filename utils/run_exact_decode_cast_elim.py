#!/usr/bin/env python3
"""Reciprocal runner: tip (shared-RoPE + device state) ± decode cast/copy elim."""
...
def run(
    *,
    ...
    from osuT5.osuT5.inference.optimized.scout.decode_cast_elim import (
    ...
    cast_context = (
        decode_cast_elim_candidate_context()
        if mode == "candidate"
        else nullcontext(DecodeCastElimActivation())
    )
    ...
    try:
        with _stacked(
            device_sequence_state_candidate_context(),
            cast_context,
        ) as (device_activation, cast_activation):
    ...
    if rng_after_seed is None:
        raise RuntimeError("decode cast-elim inference never initialized RNG")
    payload = _activation_payload(
        mode=mode,
        decode_loop_calls=int(device_activation.calls),
        cast_elim_calls=int(cast_activation.calls),
        rng_after_seed=rng_after_seed,
        rng_after_inference=rng_after_inference,
    )
