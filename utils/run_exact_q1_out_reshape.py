#!/usr/bin/env python3
"""Reciprocal exact q1 attn-out reshape on shared-RoPE + device-state tip."""
...
def _activation_payload(
    *,
    ...
    if mode not in {"baseline", "candidate"}:
        raise ValueError("q1-out-reshape mode must be baseline or candidate")
    if isinstance(calls, bool) or not isinstance(calls, int) or calls < 0:
        raise RuntimeError("q1-out-reshape decode-loop count is invalid")
    if calls <= 0:
        raise RuntimeError("q1-out-reshape requires device-sequence-state decode calls")
    return {
        "schema_version": 1,
        "role": mode,
        "candidate": "exact_device_state_plus_q1_out_reshape",
        ...
        "q1_out_reshape_enabled": mode == "candidate",
...
def run(
    *,
    ...
    from osuT5.osuT5.inference.optimized.kernels.q1_out_reshape_activation import (
    ...
    try:
        with ExitStack() as stack:
            ...
            if mode == "candidate":
                stack.enter_context(q1_out_reshape_candidate_context())
    ...
    if rng_after_seed is None:
        raise RuntimeError("q1-out-reshape inference never initialized the RNG environment")
