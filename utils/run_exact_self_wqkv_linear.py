#!/usr/bin/env python3
"""Reciprocal exact native self-Wqkv rectangular one-token linear on tip."""
...
def _activation_payload(
    *,
    ...
    if mode not in {"baseline", "candidate"}:
        raise ValueError("self-wqkv-linear mode must be baseline or candidate")
    if isinstance(calls, bool) or not isinstance(calls, int) or calls < 0:
        raise RuntimeError("self-wqkv-linear decode-loop count is invalid")
    if calls <= 0:
        raise RuntimeError(
            "self-wqkv-linear requires device-sequence-state decode calls"
        )
    return {
        "schema_version": 1,
        "role": mode,
        "candidate": "exact_device_state_plus_self_wqkv_linear",
        ...
        "self_wqkv_linear_enabled": mode == "candidate",
        "self_wqkv_linear_outputs_per_block": (
...
def run(
    *,
    ...
    from osuT5.osuT5.inference.optimized.kernels.self_wqkv_linear import (
    ...
    try:
        with ExitStack() as stack:
            ...
            if mode == "candidate":
                stack.enter_context(self_wqkv_linear_candidate_context())
    ...
    if rng_after_seed is None:
        raise RuntimeError(
            "self-wqkv-linear inference never initialized the RNG environment"
        )
    payload = _activation_payload(
        ...
        rng_after_seed=rng_after_seed,
        rng_after_inference=rng_after_inference,
        outputs_per_block=TIP_DEFAULT_OUTPUTS_PER_BLOCK,
    )
