"""Real-tensor hot/cold component gate for the opt-in SM75 DP4A projections."""
...
def _pack_layers(
    ...
    from osuT5.osuT5.inference.optimized.scout.int8_dp4a_projection import (
        validate_dp4a_pack,
    )
    ...
    for layer in layers:
        ...
        validation = {
            name: validate_dp4a_pack(pack) for name, pack in candidates.items()
        }
        packs[id(layer)] = {
...
def _candidate_functions(
    ...
    *,
    ...
    module = capture.module
    return {
        "self_norm_qkv": lambda: dp4a_rmsnorm_linear(
            inputs["layer_input"],
            module.self_attn_layer_norm.weight,
        ...
        "cross_norm_q": lambda: dp4a_rmsnorm_linear(
            inputs["after_self"],
            module.cross_attn_layer_norm.weight,
...
def summarize_component(
    ...
    *,
    ...
    if measured_replays <= 0 or total_replays < measured_replays:
        raise ValueError("invalid DP4A replay counts")
...
def profile_component(
    ...
    *,
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("DP4A projection profiler requires CUDA")
    ...
    diagnostics.record("dp4a_component_scout_start")
    ...
    for name, preload in (
        ...
        ("sm75_dp4a_candidate", preload_dp4a_projection_extension),
    ):
    ...
    retained._cuda_sync_checkpoint(diagnostics, "dp4a_projection.pack.complete")
    ...
    for prefix, accepted in selected.items():
        ...
        if packs is None:
            raise RuntimeError("captured layer has no owned DP4A pack")
        ...
        for region in REGIONS:
            ...
            for config, (warps, ctas) in CONFIGS.items():
                output, quantized, activation_scale = dp4a_rmsnorm_linear_with_state(
                    candidate_input,
                    norm_weight,
                    pack,
                    eps=eps,
    ...
    return {
        "schema_version": 1,
        "metadata": {
            "candidate": "persistent_sm75_dynamic_activation_int8_dp4a_self_qkv_cross_q",
...
def _text(report: dict[str, Any]) -> str:
    ...
    lines = [
        "SM75 persistent dynamic-activation DP4A projection component scout",
...
def main() -> None:
    ...
    if not report["summary"]["component_pass"]:
        raise SystemExit("STOP_DP4A_PROJECTION_COMPONENT")
