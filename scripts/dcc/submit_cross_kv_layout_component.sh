#!/usr/bin/env bash
# Submit the ContiguousKv / XQA-shape cross-KV layout component scout.
# Usage (from the clean experiment worktree on DCC):
#   MAPPERATORINATOR_REMOTE=islamtayeb ./scripts/dcc/submit_cross_kv_layout_component.sh
set -euo pipefail
...
# Default excludes authoritative confirmation (h36-9). Widen if nodes are busy.
...
JOB_ID=$(sbatch --parsable \
  ...
  --exclude="$EXCLUDE" \
  "$REPO/scripts/dcc/profile_cross_kv_layout_component.sbatch")
echo "submitted job_id=$JOB_ID branch=$BRANCH commit=$COMMIT exclude=$EXCLUDE"
echo "logs: $WORK/logs/cross-kv-layout-component-${JOB_ID}.out"
echo "run:  $WORK/runs/cross-kv-layout-component-${JOB_ID}/component.json"
