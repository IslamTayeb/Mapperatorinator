...
PROBE_SCRIPT=${MAPPERATORINATOR_PROBE_SCRIPT:-scripts/dcc/probe_conditional_while.sbatch}
...
command -v sbatch >/dev/null 2>&1 || fail "sbatch is unavailable; run on a DCC login node"
command -v sinfo >/dev/null 2>&1 || fail "sinfo is unavailable; run on a DCC login node"
command -v squeue >/dev/null 2>&1 || fail "squeue is unavailable; run on a DCC login node"
[[ -z "$(squeue -h -u "$USER")" ]] || {
  squeue -u "$USER" >&2
...
[[ "$PROBE_SCRIPT" != /* && "$PROBE_SCRIPT" != *".."* ]] \
  || fail "MAPPERATORINATOR_PROBE_SCRIPT must be repository-relative"
[[ -f "$REPO/$PROBE_SCRIPT" ]] || fail "missing probe script $PROBE_SCRIPT"
