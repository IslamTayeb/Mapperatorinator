#!/usr/bin/env python3
"""T2 full-map wins cell: tiger-base ms/map-token before/after.

Primary: ms_per_map_token = e2e_wall_exclude_load_jit / map_tokens
Report: main_tps, cold_start_seconds (load_jit_proxy).
No turbo. No silent capture latch (jobs force SDPA; fallback env unset).
"""
...
# Reuse W1 tiger overlay measurement (CUDA-synced generate wall).
TIGER_OVERLAY = r'''
import atexit
import json
import os
...
def main() -> None:
    ...
    # T2 gate: never silent-latch; capture must succeed under SDPA.
    ...
    if args.variant == "t2":
        overrides.extend(
            [
                ...
                # Coarser timing windows: stride 0.4 vs default 0.1 → ~4× fewer timing windows.
                "timing_lookback=0.3",
    ...
    # Prefer map-token count from MAP-labeled calls (case-insensitive).
    map_tokens = 0
    map_model_s = 0.0
