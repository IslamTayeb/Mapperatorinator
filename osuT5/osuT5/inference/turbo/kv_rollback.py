"""O(1) keep-accepted KV rollback for sampled turbo (§47 W-KV on tiger StaticCache).

StaticCache.get_seq_length is nonzero-occupancy based, so rejected slots in
``[new_len, occupied_end)`` must be cleared. That band is ≤γ (fixed small).
``cache_position`` rewind on bucketed StaticCache is the commit mechanism —
