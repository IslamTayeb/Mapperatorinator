...
TIMING_NATIVE_SELF_SCOUT_VERSION = "fp32-timing-native-self-v1"


def _timing_native_self_scout_metadata() -> dict[str, Any]:
    return {
        "version": TIMING_NATIVE_SELF_SCOUT_VERSION,
    ...
    *,
    ...
    if timing_native_self_owner is not None:
        if timing_native_self_owner is not model:
            raise RuntimeError(
                "FP32 timing-native-self scout is bound to a different model"
            )
        if preset.precision != "fp32" or model.dtype != torch.float32:
            raise TypeError(
                "timing-native-self scout requires FP32 model storage"
            )
        if approximate_weight_only_state is not None:
            raise RuntimeError(
                "timing-native-self scout cannot share a runtime with mixed weights"
            )
        if context_type != ContextType.TIMING:
            raise RuntimeError(
                "timing-native-self scout runtime may execute only timing context"
            )
    ...
    if timing_native_self_owner is not None:
        stats["optimized_dispatch_policy"]["timing_native_self_scout"] = {
            "requested": True,
        ...
        stats["optimized_timing_native_self_scout"] = (
            _timing_native_self_scout_metadata()
        )
    if approximate_weight_only_state is not None:
...
class OptimizedSingleRuntime:
    ...
    def initialize_fp32_timing_native_self(
        ...
        """Opt one FP32 timing model into the accepted native self kernels."""
        if self.preset.precision != "fp32":
            raise TypeError("timing-native-self scout requires the FP32 preset")
        if not isinstance(model, torch.nn.Module):
            raise TypeError("timing-native-self scout requires a torch module")
        if model.dtype != torch.float32:
            raise TypeError("timing-native-self scout requires FP32 model storage")
        if self._approximate_weight_only_state is not None:
            raise RuntimeError(
                "timing-native-self scout requires a timing model without mixed weights"
            )
        existing = self._timing_native_self_owner
        if existing is not None and existing is not model:
            raise RuntimeError(
                "timing-native-self scout is already bound to a different model"
            )
        object.__setattr__(self, "_timing_native_self_owner", model)
        return _timing_native_self_scout_metadata()
    ...
    def initialize_approximate_int8_mlp_weight_only(
        ...
        if existing is not None:
            ...
            if existing.cross_mode != "accepted":
                raise RuntimeError(
                    "INT8-only initialization cannot reuse a cross-candidate state"
                )
            return existing.metadata()
        from ..kernels.weight_only_runtime import ApproximateWeightOnlyState
    ...
    def initialize_approximate_int8_mlp_weight_only_cross(
        self,
        model: torch.nn.Module,
        *,
        mode: str,
    ) -> dict[str, Any]:
        """Compose one cross candidate with the explicit INT8 MLP overlay."""
        if self.preset.precision != "fp32":
        ...
        if existing is not None:
            existing.validate_owner(model)
            if not existing.int8_mlp_enabled or existing.cross_mode != mode:
                raise RuntimeError(
                    "optimized runtime already owns a different mixed-weight "
                    "INT8/cross composition"
                )
            return existing.metadata()
    ...
    def profile_metadata(self) -> dict[str, Any]:
        ...
        if self._timing_native_self_owner is not None:
            metadata["optimized_timing_native_self_scout"] = (
                _timing_native_self_scout_metadata()
            )
            metadata["optimized_result_class"] = "documented-drift"
        return metadata
