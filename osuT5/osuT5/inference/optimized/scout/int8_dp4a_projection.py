"""Opt-in SM75 DP4A projection scout.

The extension is intentionally cold on import and is not reachable from production
dispatch.  It dynamically quantizes one FP32 RMS-normalized activation vector,
then evaluates a per-output-row INT8 matrix with signed DP4A and INT32 reduction.
Outputs, dequantization, scales, and bias remain FP32.
"""
...
_DP4A_PROJECTION_EXTENSION = None
...
std::vector<torch::Tensor> dp4a_rmsnorm_linear(
    ...
    TORCH_CHECK(hidden_dim == HIDDEN_DIM,
        "signed DP4A scout requires hidden_dim=768");
    TORCH_CHECK(output_dim == 768 || output_dim == 2304,
        "signed DP4A scout supports only cross-Q or self-QKV output shapes");
...
def _load_dp4a_projection_extension():
    global _DP4A_PROJECTION_EXTENSION
    if _DP4A_PROJECTION_EXTENSION is None:
        _DP4A_PROJECTION_EXTENSION = _load_inline(
            name="mapperatorinator_sm75_dp4a_projection_scout_v1",
    ...
    return _DP4A_PROJECTION_EXTENSION


def preload_dp4a_projection_extension():
    """Build/load the verifier-only extension without changing dispatch."""

    return _load_dp4a_projection_extension()
...
def _validate_inputs(
    ...
    *,
    ...
    if hidden_dim != 768:
        raise ValueError("signed DP4A scout requires hidden_dim=768")
    ...
    if pack.weight.shape[0] not in (768, 2304):
        raise ValueError("DP4A pack must be cross-Q [768,768] or self-QKV [2304,768]")
...
def validate_dp4a_pack(pack: Int8PackedLinear) -> dict[str, float | bool]:
    ...
    if contains_minus_128:
        raise ValueError("signed DP4A pack must not contain -128")
    if pack.bias is not None:
...
def dp4a_rmsnorm_linear_with_state(
    input: torch.Tensor,
    ...
    *,
    ...
    if capability != (7, 5):
        raise RuntimeError(f"signed DP4A scout requires SM75, got sm_{capability[0]}{capability[1]}")
    values = _load_dp4a_projection_extension().dp4a_rmsnorm_linear(
        input,
        norm_weight,
        pack.weight,
        pack.scale,
    ...
    if not isinstance(values, (tuple, list)) or len(values) != 3:
        raise RuntimeError("DP4A extension returned an invalid output tuple")
    ...
    if output.dtype != torch.float32 or tuple(output.shape) != expected_output_shape:
        raise RuntimeError("DP4A output shape/dtype contract failed")
    if quantized.dtype != torch.int8 or tuple(quantized.shape) != (input.shape[2],):
        raise RuntimeError("DP4A quantized activation shape/dtype contract failed")
    if activation_scale.dtype != torch.float32 or tuple(activation_scale.shape) != (1,):
        raise RuntimeError("DP4A activation scale shape/dtype contract failed")
    if output.device != input.device or quantized.device != input.device:
        raise RuntimeError("DP4A extension returned tensors on the wrong device")
    if activation_scale.device != input.device:
        raise RuntimeError("DP4A activation scale uses the wrong device")
    return output, quantized, activation_scale


def dp4a_rmsnorm_linear(
    input: torch.Tensor,
    norm_weight: torch.Tensor,
    pack: Int8PackedLinear,
    *,
    ...
    return dp4a_rmsnorm_linear_with_state(
        input,
        norm_weight,
        pack,
        eps=eps,
...
__all__ = [
    "dp4a_rmsnorm_linear",
    "dp4a_rmsnorm_linear_with_state",
