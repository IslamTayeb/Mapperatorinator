        ...
        **kwargs,
        ...
        if self.config._attn_implementation == "flash_attention_2":
            ...
            if self.is_cross_attention:
                ...
                runtime_hooks = attention_runtime_hooks()
                optimized_self_attention_forward = (
                    runtime_hooks.q1_rope_cache_self_attention_forward
                )
                optimized_attn_outputs = None
                if optimized_self_attention_forward is not None:
                    optimized_attn_outputs = optimized_self_attention_forward(
                        module=self,
                        ...
                        sliding_window_mask=kwargs.get("sliding_window_mask"),
                    )
...
class VarWhisperDecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        self_wo_linear_forward = (
            decoder_layer_runtime_hooks().self_wo_linear_forward
        )
        if self_wo_linear_forward is not None:
            projected = self_wo_linear_forward(
                module=self,
            ...
            # Residual always separate from Wo (unlike residual-fuse §6).
            if profile_ranges:
            ...
            raise RuntimeError(
                "skip_out_proj requires decoder_layer self_wo_linear_forward"
            )
        elif profile_ranges:
            with profile_range(f"{layer_name}.self.residual"):
                hidden_states = residual + hidden_states
