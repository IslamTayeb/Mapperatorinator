...
- Do not reintroduce rejected exact-calculation experiments such as stateful monotonic time masking, final-position-only logits projection, static-cache SDPA prefix trimming, or dynamic/default-cache generation unless new profiler evidence explains why the old negative or non-equivalent result no longer applies.
